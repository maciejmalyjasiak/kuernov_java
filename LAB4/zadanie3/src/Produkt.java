...
public class Produkt {
	...
	public Produkt(int id, String data_wydania, String autor, String wydawnictwo, String tytul, double cena, Klient wPosiadaniu) {
		this.id = id;
		...
		this.tytul = tytul;
		this.cena = cena;
		this.wypozyczProdukt(wPosiadaniu, 14);
	}
			...
			if (klient.getKara() == 0) {  //sprawdzenie czy klient nie ma kary
				if (!klient.getCzyWypozyczone()) { // sprawdzenie czy klient ma wypozyczony produkt
					if (dlugosc_wypozyczenia <= 14) { // czy klient stara sie wypozyczyc produkt na wiecej niz 14 dni
						...
						System.out.println("Nie mozna wyporzyczyc na wiecej niz 2 tygodnie");
					}
				} else {
